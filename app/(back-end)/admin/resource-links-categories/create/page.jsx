...
export default function Page({initialData}) {
  ...
  <div className='flex flex-col gap-2'>
    ...
    <div className="max-w-screen-xl mx-auto px-4 py-3 items-center justify-between text-white sm:flex md:px-8">
        <div className="flex gap-x-4">
            ...
            <p className="lg:py-2 py-4 font-medium text-sm ">
              These categories belong to Resource links
            </p>
        </div>
        <Link href="/admin/resource-links-categories" className="inline-block w-full mt-3 py-2 px-3 text-center text-indigo-600 font-medium bg-white duration-150 hover:bg-gray-100 active:bg-gray-200 rounded-lg sm:w-auto sm:mt-0 sm:text-sm">
         Link Categories
        </Link>
    </div>
     </div>
    ...
    >
      <TextInputs
        placeholder='Enter Link Category title'
        label='Enter Link Category title'
        name='title'
        type='text'
        errors={errors}
        register={register}
      ...
      <div>
        {loading ? (
          ...
          <Button type='submit'>Create Link category </Button>
        )}
      </div>
    </form>
  </div>
