...
export function SubCategoriesForm({ initialData, categories }) {
  ...
  async function onSubmit(data) {
    ...
    const apiUrl = initialData
      ? `${baseUrl}/api/subCategory/update/${initialData.id}`
      ...
      <div>
        {loading ? (
          <button
            disabled
            type='button'
            className='text-white bg-pink-500 hover:bg-pink-600 focus:ring-4 focus:outline-none font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 dark:bg-pink-500 dark:hover:bg-pink-500 dark:focus:ring-pink-600 inline-flex items-center'
          >
            <svg
              aria-hidden='true'
              role='status'
