...
import adminApiRequest from "@/utils/adminApiRequest"
  ...
  async function onSubmit(data) {
    ...
    const apiUrl =  initialData ? `${baseUrl }/api/update/${initialData.id}` :`${baseUrl}/api/notes`;
    // console.log(data)
    adminApiRequest({
      setLoading2,
      setLoading,
  ...
  return (
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col space-y-8 w-full lg:w-[80%] mt-4 border border-gray-900 p-5 rounded-lg">
...
<SelectInput
      ...
      />
        ...
        formats={formats} theme="snow" value={value} onChange={setValue} className='w-full' />
              <div className="mt-8 prose hidden" dangerouslySetInnerHTML={{ __html: value }} />
            </div>
          </div>
